...
class Archivos {
	...
	private function nombreRutaArchivos ($dateServidor , $extension , $nombre)
	{
		...
		$nombreAux = Carbon :: createFromFormat ( 'Y-m-d H:i:s',
		$dateServidor ) -> format ( 'H-i-s' ) . $nombre;
		return Auth :: user ( ) -> tipo_usuario . '/' . $extension . '/' .
		 $nombreAux;
	...
	private function nombreRutaArchivo ($dateServidor)
	{
		...
		return $this -> nombreRutaArchivos ( $dateServidor,
	...
	public function guardarArchivo ($dateServidor)
	{
		...
		$this -> nombreRutaArchivo ( $dateServidor ),
		File :: get ( $this -> file ) );
	...
	public function quitarExtension ( )
	{
		...
		return basename ( $this -> file -> getClientOriginalName ( ),
		'.' . $this -> file -> getClientOriginalExtension ( ) );
	...
	public function copiarArchivo ($dateServidor , $newDate , $nombre , $extension , $cambioNombre)
	{
		...
		$rutaCompleta = $this -> nombreRutaArchivos ( $dateServidor,
		$extension, $nombre ) . "." . $extension;
		$rutaCompletaNueva = $this -> nombreRutaArchivos ( $newDate,
		$extension, $cambioNombre ) . "." . $extension;
	...
	public function eliminarArchivo ($dateServidor , $nombre , $extension)
	{
		...
		$this -> nombreRutaArchivos ( $dateServidor, $extension, $nombre ) .
		 "." . $extension );
